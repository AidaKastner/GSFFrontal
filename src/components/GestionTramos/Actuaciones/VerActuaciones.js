...
class VerActuaciones extends Component{
  ...
  this.columns = [
    ...
    {dataField: 'calzada', text: <Translation ns= "global">{(t) => <>{t('Calzada')}</>}</Translation>, sort: true, filter: textFilter({placeholder: ' '}), formatter: (cell, row) =>{return <div style={{display: 'inline-block', textAlign: 'left'}}>{`${row.carriles != ''? ((row.carriles?.substring(0,1) > 0 && row.carriles?.substring(2,3) > 0) ? 'Separades' : 'Única') :''}`}</div>;}, style: this.columnStyle},
