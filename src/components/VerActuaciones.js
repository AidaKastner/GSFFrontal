...
class VerActuaciones extends Component{
  ...
  this.columns = [
    ...
    {dataField: 'fecha', text: <Translation ns= "global">{(t) => <>{t('Fecha')}</>}</Translation>, sort: true, formatter: (cell, row) =>{return <div>{`${row.fecha != null ? row.fecha.substring(0,10) : row.fecha}`}</div>;}, filter: textFilter({placeholder: ' '}), style: this.columnStyle, formatter: this.formatText},
    {dataField: 'sentido', text: <Translation ns= "global">{(t) => <>{t('Sentido')}</>}</Translation>, sort: true, filter: textFilter({placeholder: ' '}), formatter: (cell, row) =>{return <div>{`${row.sentido == 'C'? 'Creixent': row.sentido == 'D'? 'Decreixent' : 'Creixent/Decreixent'}`}</div>;}, style: this.columnStyle, formatter: this.formatText},
    {dataField: 'calzada', text: <Translation ns= "global">{(t) => <>{t('Calzada')}</>}</Translation>, sort: true, filter: textFilter({placeholder: ' '}), formatter: (cell, row) =>{return <div>{`${row.carriles != ''? ((row.carriles?.substring(0,1) > 0 && row.carriles?.substring(2,3) > 0) ? 'Separades' : 'Única') :''}`}</div>;}, style: this.columnStyle, formatter: this.formatText},
    ...
    {dataField: 'puntoIni.pk', text: <Translation ns= "global">{(t) => <>{t('PKIni')}</>}</Translation>, formatter: (cell, row) =>{return <div>{`${row.puntoIni.pk} + ${row.puntoIni.m}`}</div>;}, filter: textFilter({placeholder: ' '}), style: this.columnStyle, formatter: this.formatText},
    {dataField: 'puntoFin.pk', text: <Translation ns= "global">{(t) => <>{t('PKFin')}</>}</Translation>, formatter: (cell, row) =>{return <div>{`${row.puntoFin.pk} + ${row.puntoFin.m}`}</div>;}, filter: textFilter({placeholder: ' '}), style: this.columnStyle, formatter: this.formatText},
    {dataField: 'importe', text: <Translation ns= "global">{(t) => <>{t('Importe')}</>}</Translation>, sort: true, filter: textFilter({placeholder: ' '}), formatter: (cell, row) =>{return <div>{`${row.importe?.toLocaleString('es')}`}</div>;}, style: this.columnStyle, formatter: this.formatText}
  ]
